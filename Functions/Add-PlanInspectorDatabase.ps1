Function Add-PlanInspectorDatabase
{
    [CmdletBinding()]
    Param (
...
$CreateDatabase = "CREATE DATABASE [{@PlanInspector_Database}]"
$CreateDatabase = ($CreateDatabase.Replace("{@PlanInspector_Database}","$($PlanInspector_Database)"))
Write-Verbose "Creating database"
Write-Verbose $CreateDatabase
Invoke-SqlCmd -ServerInstance $PlanInspector_Instance -Database master -Query $CreateDatabase @EncryptionParameter
}