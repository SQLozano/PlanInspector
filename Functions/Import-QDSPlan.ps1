Function Import-QDSPlan
{
    ...
    Param (
        ...
        [Parameter(Mandatory=$true)][string]$Target_Database,
        [Parameter(Mandatory=$false)][int]$Target_PlanID,
        [Parameter(Mandatory=$false)][string]$Target_Object,
        [Parameter(Mandatory=$false)][int]$Target_MinPlanID,
...
,[object_type]				=	COALESCE(o.type_desc, 'Dynamic SQL')
...
,[showplan_xml]				=	NULL
,[showplan_xml_compressed]	=	COMPRESS(qsp.query_plan)
,[database_name]			=	DB_NAME()
,[transaction_id]			=	NULL
,[sql_text]					=	NULL
,[sql_text_compressed]		=	COMPRESS(qsqt.query_sql_text)
...
--{@Filter}"
Write-Verbose "Connecting to [$Target_Instance] \ [$Target_Database] to extract QDS data"
if($Target_PlanID){
    $QueryQDSData = ($QueryQDSData.Replace("--{@Filter}","WHERE qsp.plan_id = $($Target_PlanID)"))
    Write-Verbose "Target_PlanID = $($Target_PlanID)"
}
if($Target_Object){
    $QueryQDSData = ($QueryQDSData.Replace("--{@Filter}","WHERE s.name + '.' + o.name = $($Target_Object)"))
    Write-Verbose "Target_Object = $($Target_Object)"
}
if($Target_MinPlanID){
    $QueryQDSData = ($QueryQDSData.Replace("--{@Filter}","WHERE qsp.plan_id > $($Target_MinPlanID)"))
    Write-Verbose "Target_PlanID > $($Target_MinPlanID)"
}
...
$InstanceCommand.CommandTimeout = 300000
...
$ProcessedColumn = [System.Data.DataColumn]::new('Processed', [boolean])
$ProcessedColumn.DefaultValue = $false
$QueryResults.Columns.Add($ProcessedColumn)
...
Write-SqlTableData -ServerInstance $PlanInspector_Instance -Database $PlanInspector_Database -SchemaName $PlanInspector_Schema -TableName $PlanInspector_Table -InputData $QueryResults -Timeout 600
